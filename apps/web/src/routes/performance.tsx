...
function PerformanceTestPage() {
  ...
  const fetchPokemons = async () => {
    ...
    try {
      ...
      const pokemonData: PokemonData[] = (result?.results || []).map(
        (p: any) => {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Affichez tous les Pokémon avec leurs informations de base
          </p>
        </div>
        <Button onClick={fetchPokemons} disabled={isLoading}>
          {isLoading ? "Chargement..." : "Charger les Pokémon"}
        </Button>
      </div>
      ...
      {duration !== null && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Temps de chargement des données</CardDescription>
          </CardHeader>
          <CardContent>
